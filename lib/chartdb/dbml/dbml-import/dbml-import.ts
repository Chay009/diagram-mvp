...
import type { DataTypeData } from '@/lib/chartdb/data/data-types/data-types';
import {
    findDataTypeDataById,
    requiresNotNull,
} from '@/lib/chartdb/data/data-types/data-types';
...
import { getTableIndexesWithPrimaryKey, type DBIndex } from '@/lib/chartdb/domain';
import {
    DBCustomTypeKind,
    type DBCustomType,
} from '@/lib/chartdb/domain/db-custom-type';
...
export const preprocessDBML = (content: string): PreprocessDBMLResult => {
    ...
    processed = processed.replace(/TableGroup\s+[^{]*\{[^}]*\}/g, '');

    // Remove Note blocks
    processed = processed.replace(/Note\s+\w+\s*\{[^}]*\}/g, '');

    // Don't remove enum definitions - we'll parse them
    // processed = processed.replace(/enum\s+\w+\s*\{[^}]*\}/g, '');
